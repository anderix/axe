...
The viewer renders document content as live HTML in your site's origin, so treat every document you point it at as code. Markdown is sanitized with DOMPurify and calendar event URLs are scheme-checked before they become links, but those are mitigations, not a license to render untrusted input freely. External `?url=` fetches are denied by default, and the `view/list.php` directory lister is confined to a configurable subtree. Before you deploy the viewer, read [SECURITY.md](SECURITY.md) — it covers the threat model, the `EXTERNAL_ALLOWLIST` and `$confine` knobs, and the operator responsibilities the framework cannot enforce for you.
